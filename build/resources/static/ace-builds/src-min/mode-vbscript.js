...
define("ace/mode/vbscript_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function (e, t, n) {
    ...
    r.inherits(u, i), function () { this.indentKeywords = { "class": 1, "function": 1, sub: 1, "if": 1, select: 1, "do": 1, "for": 1, "while": 1, "with": 1, property: 1, "else": 1, elseif: 1, end: -1, loop: -1, next: -1, wend: -1 }, this.foldingStartMarker = /(?:\s|^)(class|function|sub|if|select|do|for|while|with|property|else|elseif)\b/i, this.foldingStopMarker = /\b(end|loop|next|wend)\b/i, this.getFoldWidgetRange = function (e, t, n) { var r = e.getLine(n), i = this.foldingStartMarker.test(r), s = this.foldingStopMarker.test(r); if (i || s) {
        var o = s ? this.foldingStopMarker.exec(r) : this.foldingStartMarker.exec(r), u = o && o[1].toLowerCase();
        if (u) {
    ...
    } }, this.getFoldWidget = function (e, t, n) { var r = e.getLine(n), i = this.foldingStartMarker.test(r), s = this.foldingStopMarker.test(r); if (i && !s) {
        var o = this.foldingStartMarker.exec(r), u = o && o[1].toLowerCase();
        if (u) {
            var a = e.getTokenAt(n, o.index + 2).type;
            if (a == "keyword.control.asp" || a == "storage.type.function.asp")
                return u == "if" && !/then\s*('|$)/i.test(r) ? "" : "start";
        }
    } return ""; }, this.vbsBlock = function (e, t, n, r) { var i = new o(e, t, n), u = { "class": 1, "function": 1, sub: 1, "if": 1, select: 1, "with": 1, property: 1, "else": 1, elseif: 1 }, a = i.getCurrentToken(); if (!a || a.type != "keyword.control.asp" && a.type != "storage.type.function.asp")
        return; var f = a.value.toLowerCase(), l = a.value.toLowerCase(), c = [l], h = this.indentKeywords[l]; if (!h)
        return; var p = i.getCurrentTokenRange(); switch (l) {
        case "property":
        case "sub":
        case "function":
        case "if":
        ...
        case "while":
        case "with":
            var d = e.getLine(t), v = /^\s*If\s+.*\s+Then(?!')\s+(?!')\S/i.test(d);
            if (v)
                return;
            var m = new RegExp("(?:^|\\s)" + l, "i"), g = /^\s*End\s(If|Sub|Select|Function|Class|With|Property)\s*/i.test(d);
            if (!m.test(d) && !g)
                return;
            if (g) {
        ...
        case "end":
            ...
            if (a) {
                ...
                if (l in u) {
                    f = l;
                    var b = i.getCurrentTokenPosition(), w = b.column + l.length;
                    p = new s(y.row, y.column, b.row, w);
                }
            }
            i.step = i.stepBackward, i.step(), i.step();
    } var E = h === -1 ? e.getLine(t - 1).length : e.getLine(t).length, S = t, x = []; x.push(p), i.step = h === -1 ? i.stepBackward : i.stepForward; while (a = i.step()) {
        var T = null, N = !1;
        ...
        switch (l) {
            ...
            case "while":
            case "with":
                var d = e.getLine(i.getCurrentTokenRow()), v = /^\s*If\s+.*\s+Then(?!')\s+(?!')\S/i.test(d);
                v && (C = 0, N = !0);
                var m = new RegExp("^\\s* end\\s+" + l, "i");
                m.test(d) && (C = 0, N = !0);
                break;
            case "elseif":
            case "else": C = 0, f != "elseif" && (N = !0);
        }
        if (C > 0)
            c.unshift(l);
        else if (C <= 0 && N === !1) {
        ...
        return null; if (r)
        return T ? x.push(T) : x.push(i.getCurrentTokenRange()), x; var t = i.getCurrentTokenRow(); if (h === -1) {
        var w = e.getLine(t).length;
        return new s(t, w, S - 1, E);
    } return new s(S, E, t - 1, e.getLine(t - 1).length); }; }.call(u.prototype);
    ...
    "use strict";
    var r = e("../lib/oop"), i = e("./text").Mode, s = e("./vbscript_highlight_rules").VBScriptHighlightRules, o = e("./folding/vbscript").FoldMode, u = e("../range").Range, a = function () { this.HighlightRules = s, this.foldingRules = new o, this.$behaviour = this.$defaultBehaviour, this.indentKeywords = this.foldingRules.indentKeywords; };
    r.inherits(a, i), function () { function t(e, t, n) { var r = 0; for (var i = 0; i < e.length; i++) {
        var s = e[i];
        if (s.type == "keyword.control.asp" || s.type == "storage.type.function.asp") {
            ...
            if (o in n)
                switch (o) {
                    ...
                    case "with":
                    case "if":
                        var u = new RegExp("^\\s* end\\s+" + o, "i"), a = /^\s*If\s+.*\s+Then(?!')\s+(?!')\S/i.test(t);
                        !a && !u.test(t) && (r += n[o]);
                        break;
                    default: r += n[o];
                }
        }
    } return r < 0 ? -1 : r > 0 ? 1 : 0; } this.lineCommentStart = ["'", "REM"]; var e = ["else", "elseif", "end", "loop", "next", "wend"]; this.getNextLineIndent = function (e, n, r) { var i = this.$getIndent(n), s = 0, o = this.getTokenizer().getLineTokens(n, e), u = o.tokens; return e == "start" && (s = t(u, n, this.indentKeywords)), s > 0 ? i + r : s < 0 && i.substr(i.length - r.length) == r && !this.checkOutdent(e, n, "\n") ? i.substr(0, i.length - r.length) : i; }, this.checkOutdent = function (t, n, r) { if (r != "\n" && r != "\r" && r != "\r\n")
        return !1; var i = this.getTokenizer().getLineTokens(n.trim(), t).tokens; if (!i || !i.length)
        return !1; var s = i[0].value.toLowerCase(); return (i[0].type == "keyword.control.asp" || i[0].type == "storage.type.function.asp") && e.indexOf(s) != -1; }, this.getMatching = function (e, t, n, r) { if (t == undefined) {
        var i = e.selection.lead;
        n = i.column, t = i.row;
    } r == undefined && (r = !0); var s = e.getTokenAt(t, n); if (s) {
        var o = s.value.toLowerCase();
        if (o in this.indentKeywords)
            return this.foldingRules.vbsBlock(e, t, n, r);
    } }, this.autoOutdent = function (e, t, n) { var r = t.getLine(n), i = r.match(/^\s*/)[0].length; if (!i || !n)
        return; var s = this.getMatching(t, n, i + 1, !1); if (!s || s.start.row == n)
        return; var o = this.$getIndent(t.getLine(s.start.row)); o.length != i && (t.replace(new u(n, 0, n, i), o), t.outdentRows(new u(n + 1, 0, n + 1, 0))); }, this.$id = "ace/mode/vbscript"; }.call(a.prototype), t.Mode = a;
});
(function () {
